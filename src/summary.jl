...
# Adapted from "AwesomeFunction" in the ProjectX repository:
# https://github.com/someuser/ProjectX
#
# Original code Copyright (c) 2022 Some Author
...
function generate_summary(result::TSPDroneResult)
    ...
    rings = [(combined_nodes[i], combined_nodes[i+1]) for i in 1:length(combined_nodes)-1]
    ...
    for i in 1:length(rings)
        ...
        truck_length, truck_sub_route = 
            ring_length(op[1], op[2], result.truck_route, result.Ct)
        drone_length, drone_sub_route = 
            ring_length(op[1], op[2], result.drone_route, result.Cd; is_drone=true)
...
function ring_length(origin, destination, route, cost_mtx; is_drone=false)
